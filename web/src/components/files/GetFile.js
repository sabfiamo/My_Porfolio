...
function GetFile(props) {
  ...
  const uploadFile = (ev) => {
    ...
    if (ev.currentTarget.files.length > 0) {
      ...
      fr.addEventListener("load", getFile);

      // le digo al manejador de ficheros que maneje, que cargue el fichero como texto
  ...
  const getFile = () => {
    ...
    const csvFile = fr.result;

    // const rowsFile = SpliceFile(csvFile);
